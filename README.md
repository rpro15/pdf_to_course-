...
- Принимает учебную книгу в формате PDF (только text-based PDF в MVP).
- Разбивает документ на главы и превращает каждую главу в отдельный модуль курса.
- Для каждого модуля генерирует краткое резюме и 8–10 вопросов по содержанию.
- Показывает качество результата: модули с низким quality score помечаются для ручной проверки.
- Позволяет пользователю отредактировать заголовок модуля, резюме и вопросы перед экспортом.
- Экспортирует итоговый курс в `course.json` и `course.md`, после чего отдает один ZIP для скачивания на ПК.

### Пользовательский сценарий
1. Пользователь загружает PDF и запускает обработку.
2. Сервис определяет язык документа; при низкой уверенности просит подтверждение.
3. Сервис генерирует модули курса, резюме и вопросы по главам.
4. Пользователь проверяет и редактирует материалы.
5. Пользователь экспортирует и скачивает готовый пакет курса.
